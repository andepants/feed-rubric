...
| Host permissions overreach? | Only `https://api.typesafe.ai/*`. x.com / fixture injection uses `content_scripts.matches`, not `<all_urls>` or `http://localhost/*`. |
| Cache poisoning? | Dry-run `data-feed-rubric-scores` are accepted only from `http://127.0.0.1:8080` / `:18080`, verified from `sender.url` in the service worker. Live hosts cannot spoof fixture scores. Fixture hosts never call TypeSafe. |
...
| High | Localhost content scripts could spend the user's TypeSafe quota. | Fixture origins never take the API path. Content script matches are port-pinned (`8080`, `18080`), not `localhost/*`. |
| High | `host_permissions` included x.com, twitter.com, and all localhost HTTP. | Host permission is TypeSafe only. |
